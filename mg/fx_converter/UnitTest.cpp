...
BOOST_AUTO_TEST_CASE(corr_mon_test){
    ...
    BOOST_CHECK(getExchange("/home/margon/projects/back-to-cpp/datasets/exchange_rates.csv", umap));

    BOOST_CHECK(umap["GBP"]["USD"] -> getAsk()==1.3701);
    BOOST_CHECK(umap["GBP"]["USD"] -> getBid()==1.373);

    BOOST_CHECK(corr_mon(umap,"USD","USD",100)==0);

    BOOST_CHECK(corr_mon(umap,"USD","GBP",0)==0);
